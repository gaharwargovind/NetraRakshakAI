...
def save_cm_plot(cm, out_path):
    ...
    fig.colorbar(cax)
    ax.set_title("E009 Confusion Matrix (N=601)", fontweight="bold")
...
def main():
    ...
    model = DREfficientNet(backbone_name="efficientnet_b0", num_classes=5, pretrained=True, dropout_rate=0.2).to(device)
        # Class-Balanced Alpha exclusively derived from train.csv
    train_df = pd.read_csv(ROOT_DIR / "data/processed/aptos/train.csv")
    train_counts = train_df["diagnosis"].value_counts().sort_index().to_dict()
    total_train = len(train_df)
    alpha_weights = [round(float(total_train / (5.0 * train_counts[c])), 4) for c in range(5)]
    print(f"Derived Class-Balanced Alpha (counts: {train_counts}):\n{alpha_weights}")

    criterion = ClassBalancedFocalLoss(gamma=2.0, alpha=alpha_weights, reduction="mean")
    optimizer = AdamW(model.parameters(), lr=3e-4, weight_decay=1e-5)
    scheduler = CosineAnnealingLR(optimizer, T_max=30)
    exp_dir = ROOT_DIR / "experiments/E009_class_balanced_focal"
    ...
    print("Beginning E009 Class-Balanced Focal Loss training...")
    for ep in range(1, 31):
        ...
        if val["macro_f1"] > best_f1:
            ...
            ckpt = {"epoch": ep, "model_state_dict": model.state_dict(), "best_metric": best_f1,
                    "config": {"experiment": "E009", "loss": "class_balanced_focal_loss", "gamma": 2.0, "alpha": alpha_weights}}
            torch.save(ckpt, ckpt_dir / "E009_best_model.pt")
    ...
    payload = {"experiment_id": "E009", "best_epoch": best_epoch, "duration": round(time.time() - start_t, 1),
    ...
    cfg_src = ROOT_DIR / "configs/experiments/class_balanced_focal.yaml"
    ...
    if reg_path.is_file():
        ...
        row = {"experiment_id": "E009", "experiment_name": "Class-Balanced Focal Loss", "backbone": "efficientnet_b0",
               ...
               "status": "VALIDATED" if sens_pass else "FAILED_SENSITIVITY", "notes": f"Single factor vs E007. Best epoch {best_epoch}."}
    ...
    print("=" * 70)
    print(f"E009 COMPLETE | Best Epoch: {best_epoch} | F1: {best_res['macro_f1']:.4f} | QWK: {best_res['quadratic_weighted_kappa']:.4f} | Sens: {best_res['referable_sensitivity']:.4f} | Spec: {best_res['referable_specificity']:.4f}")
    print("=" * 70)
